...
/**
 * This plug-in calculates taxes for invoice for norwegian companies.
 *
 * Plug-in parameters:
 *
 ...
 * @since 13.03.2013
 */
public class NorwegianTaxCompositionTask extends PluggableTask
        implements InvoiceCompositionTask {

    private static final Logger LOG = Logger.getLogger(NorwegianTaxCompositionTask.class);
    ...
    private boolean isNorwegian(Integer userId) {
        ContactDTO contactDto = new ContactDAS().findPrimaryContact(userId);
        if (contactDto == null) {
            return true;
        }
        //If not norwegian, don't add taxes
        if (!contactDto.getCountryCode().equals("NO")) {
                return false;
        }
