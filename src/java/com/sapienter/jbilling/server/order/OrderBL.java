...
public class OrderBL extends ResultList implements OrderSQL {
	...
	public OrderWS getWS(Integer languageId) {
		OrderWS retValue = new OrderWS(order.getId(), order.getBillingTypeId(),
				...
				order.getNotesInInvoice(), order.getOwnInvoice(), order
						...
						.getId(), order.getVersionNum(),
				order.getCycleStarts(), order.getIsMaster(),
				order.getAddToMaster(), order.getPayPlan());
		...
		retValue.setGeneratedInvoices(new InvoiceBL().DTOtoWS(new ArrayList(
				order.getInvoices())));
	...
	public void addItem(OrderDTO order, Integer itemID, BigDecimal quantity,
			Integer language, Integer userId, Integer entityId,
			Integer currencyId, List<Record> records)
			throws ItemDecimalsException {

		try {
			PluggableTaskManager<IItemPurchaseManager> taskManager = new PluggableTaskManager<IItemPurchaseManager>(
					entityId, Constants.PLUGGABLE_TASK_ITEM_MANAGER);
			IItemPurchaseManager myTask = taskManager.getNextClass();

			while (myTask != null) {
				myTask.addItem(itemID, quantity, language, userId, entityId,
						currencyId, order, records);
				myTask = taskManager.getNextClass();
			}

		} catch (PluggableTaskException e) {
			throw new SessionInternalError("Item Manager task error",
					OrderBL.class, e);
		} catch (TaskException e) {
			if (e.getCause() instanceof ItemDecimalsException) {
				throw (ItemDecimalsException) e.getCause();
			} else {
				// do not change this error text, it is used to identify the
				// error
				throw new SessionInternalError("Item Manager task error",
						OrderBL.class, e);
			}
	...
	public void addItem(Integer itemID, BigDecimal quantity, Integer language,
		...
		addItem(this.order, itemID, quantity, language, userId, entityId,
				currencyId, records);
	}
	...
	public void addItem(Integer itemID, Integer quantity, Integer language,
			Integer userId, Integer entityId, Integer currencyId,
			List<Record> records) throws ItemDecimalsException {
		addItem(itemID, new BigDecimal(quantity), language, userId, entityId,
				currencyId, records);
	}
	...
	public void delete(Integer executorId) {
		...
		EventManager.process(new OrderDeletedEvent(order.getBaseUserByUserId()
				.getCompany().getId(), order));

		// UPDATING THE NUMBER OF USERS FOR ITEM AND USER for deleted orders
		// (master and normal)
		...
		if ((order.getAddToMaster() != 1)) {
			...
			/*
			 * //UPDATING MASTER ORDER WHEN addToMAster deleted (quantity and
			 * price) if(order.getAddToMaster()==1){ if(master!=null){
			 * for(Iterator it=master.getLines().iterator();it.hasNext();){
			 * OrderLineDTO o=(OrderLineDTO) it.next(); Integer
			 * q=orderDas.findNumberUsers(o.getItemId(), master.getUserId());
			 * o.setQuantity(q); o.setUseItem(true); } } }
			 */
		...
		eLogger.audit(executorId, order.getBaseUserByUserId().getId(),
				...
				EventLogger.MODULE_ORDER_MAINTENANCE, EventLogger.ROW_DELETED,
				null, null, null);
	}

	/**
	 * Method recalculate. Goes over the processing tasks configured in the
	 * database for this entity. The order entity is then modified.
	 */
	public void recalculate(Integer entityId) throws SessionInternalError,
			ItemDecimalsException {
		...
		order.setBaseUserByUserId(user
				.find(order.getBaseUserByUserId().getId()));
		...
		try {
			...
			OrderProcessingTask task = (OrderProcessingTask) taskManager
					.getNextClass();
			while (task != null) {
			...
			throw new SessionInternalError(
					"Problems executing order processing task.");
		}
	}
	...
	public Integer create(Integer entityId, Integer userAgentId,
		...
		try {
			...
			if (orderDto.getOrderPeriod().getId() == Constants.ORDER_PERIOD_ONCE) {
				orderDto.setOrderBillingType(orderBillingTypeDas
						.find(Constants.ORDER_BILLING_PRE_PAID));
			...
			// create the record
			orderDto.setBaseUserByUserId(user.find(orderDto
					.getBaseUserByUserId().getId()));
			orderDto.setOrderPeriod(orderPeriodDAS.find(orderDto
			...
			for (OrderLineDTO line : orderDto.getLines()) {
				...
				if (line.getProvisioningStatus() == null) {
					line.setProvisioningStatus(provisioningStatusDas
							.find(CommonConstants.PROVISIONING_STATUS_INACTIVE));
				} else {
					line.setProvisioningStatus(provisioningStatusDas.find(line
			...
			// UPDATING THE NUMBER OF USERS FOR ITEM AND USER for new orders
			// (master and normal) If addToMaster==1 then master edited
			// automatically
			if (order.getAddToMaster() != 1) {
				for (Iterator i = order.getLines().iterator(); i.hasNext();) {
					OrderLineDTO temp = (OrderLineDTO) i.next();
			...
			if (activeSince == null || activeSince.before(now)) {
				...
				SubscriptionActiveEvent newEvent = new SubscriptionActiveEvent(
						entityId, order);
				EventManager.process(newEvent);
				LOG.debug("OrderBL.create(): generated SubscriptionActiveEvent for order: "
						+ order.getId());
			}
			...
			throw new SessionInternalError(
					"Create exception creating order entity bean",
					OrderBL.class, e);
		}
	...
	public void updateActiveUntil(Integer executorId, Date to, OrderDTO newOrder) {
		...
		// this needs an event
		NewActiveUntilEvent event = new NewActiveUntilEvent(order.getId(), to,
		...
		// if the new active until is earlier than the next invoice date, we
		// have a
		// period already invoice being cancelled
		if (isDateInvoiced(to)) {
			// pass the new order, rather than the existing one. Otherwise, the
			// exsiting gets
			// and changes overwritten by the data of the new order.
			EventManager.process(new PeriodCancelledEvent(newOrder, order
					.getBaseUserByUserId().getCompany().getId(), executorId));
		}
	}

	/**
	 * Method checkOrderLineQuantities. Creates a NewQuantityEvent for each
	 * order line that has had its quantity modified (including those added or
	 * deleted).
	 * 
	 * @return An array with all the events that should be fiered. This prevents
	 *         events being fired when the order has not be saved and it is
	 *         still 'mutating'.
	 */
	public List<NewQuantityEvent> checkOrderLineQuantities(
			List<OrderLineDTO> oldLines, List<OrderLineDTO> newLines,
			Integer entityId, Integer orderId, boolean sendEvents) {
		...
		OrderLineDTO currentOldLine = itOldLines.hasNext() ? itOldLines.next()
				: null;
		OrderLineDTO currentNewLine = itNewLines.hasNext() ? itNewLines.next()
				: null;
		while (currentOldLine != null && currentNewLine != null) {
			...
			if (oldLineId < newLineId) {
				...
				retValue.add(new NewQuantityEvent(entityId, currentOldLine
						.getQuantity(), BigDecimal.ZERO, orderId,
						currentOldLine, null));
				currentOldLine = itOldLines.hasNext() ? itOldLines.next()
						: null;
			} else if (oldLineId > newLineId) {
				// order line has been added
				LOG.debug("Added order line. Order line Id: " + newLineId);
				retValue.add(new NewQuantityEvent(entityId, BigDecimal.ZERO,
						currentNewLine.getQuantity(), orderId, currentNewLine,
						null));
				currentNewLine = itNewLines.hasNext() ? itNewLines.next()
						: null;
			} else {
				...
				if (oldLineQuantity.compareTo(newLineQuantity) != 0) {
					LOG.debug("Order line quantity changed. Order line Id: "
							+ oldLineId);
					retValue.add(new NewQuantityEvent(entityId,
							oldLineQuantity, newLineQuantity, orderId,
		...
		// check for any remaining item lines that must have been deleted or
		// added
		while (currentOldLine != null) {
			LOG.debug("Deleted order line. Order line id: "
					+ currentOldLine.getId());
			retValue.add(new NewQuantityEvent(entityId, currentOldLine
					.getQuantity(), BigDecimal.ZERO, orderId, currentOldLine,
					null));
			currentOldLine = itOldLines.hasNext() ? itOldLines.next() : null;
		}
		while (currentNewLine != null) {
			LOG.debug("Added order line. Order line id: "
					+ currentNewLine.getId());
			retValue.add(new NewQuantityEvent(entityId, BigDecimal.ZERO,
	...
	public void update(Integer executorId, OrderDTO dto) {
		...
		order.setPayPlan(dto.getPayPlan());
		...
		LOG.info("Order lines: " + order.getLines().size()
				+ "  --> new Order: " + dto.getLines().size());
		...
		if (oldLine != null && nonDeletedLines == 1) {
			...
			if (newLine != null
					&& !oldLine.getItemId().equals(newLine.getItemId())) {
				if (executorId != null) {
					eLogger.audit(executorId, order.getBaseUserByUserId()
							.getId(), Constants.TABLE_ORDER_LINE, newLine
							.getId(), EventLogger.MODULE_ORDER_MAINTENANCE,
							EventLogger.ORDER_LINE_UPDATED, oldLine.getId(),
							oldLine.getDescription(), null);
				} else {
					// it is the mediation process
					eLogger.auditBySystem(order.getBaseUserByUserId()
							.getCompany().getId(), order.getBaseUserByUserId()
							.getId(), Constants.TABLE_ORDER_LINE, newLine
							.getId(), EventLogger.MODULE_ORDER_MAINTENANCE,
							EventLogger.ORDER_LINE_UPDATED, oldLine.getId(),
							oldLine.getDescription(), null);
		...
		if (order.getIsMaster() == 1) {
			...
			for (Iterator i = order.getLines().iterator(); i.hasNext();) {
				...
				if(temp.getDeleted()==0){
					...
					recentlyDeletedLines.add(temp);
				}
				
			}
			// Find deleted lines. if deletedLineItemId not in
			// masterOrderNotDeletedLines ->
			// updateOrInsertItemUsers(deletedLineItemId,order.getUserId(),new
			// Integer(0))
	...
	private void updateNextBillableDay(Integer executorId, Date newDate) {
		...
		if (order.getNextBillableDay() == null
			...
			eLogger.audit(executorId, order.getBaseUserByUserId().getId(),
					Constants.TABLE_PUCHASE_ORDER, order.getId(),
					EventLogger.MODULE_ORDER_MAINTENANCE,
					EventLogger.ORDER_NEXT_BILL_DATE_UPDATED, null, null,
	...
	/**
	 * Method lookUpEditable. Gets the row from order_line_type for the type
	 * specifed
	 * 
	 * @param type
	 *            The order line type to look.
	 * @return Boolean If it is editable or not
	...
	static public Boolean lookUpEditable(Integer type)
		...
		try {
			...
			LOG.fatal("Exception looking up the editable flag of an order "
					+ "line type. Type = " + type, e);
	...
	public CachedRowSet getList(Integer entityID, Integer userRole,
		...
		} else if (userRole.equals(CommonConstants.TYPE_CUSTOMER)) {
	...
	public List<Integer> getOrdersByProcess(Integer processId)
			throws SQLException, Exception {
		conn = ((DataSource) Context.getBean(Context.Name.DATA_SOURCE))
				.getConnection();
	...
	public void setStatus(Integer executorId, Integer statusId) {
		...
		if (executorId != null) {
			eLogger.audit(executorId, order.getBaseUserByUserId().getId(),
					Constants.TABLE_PUCHASE_ORDER, order.getId(),
					EventLogger.MODULE_ORDER_MAINTENANCE,
					EventLogger.ORDER_STATUS_CHANGE, order.getStatusId(), null,
					null);
		} else {
			eLogger.auditBySystem(order.getBaseUserByUserId().getCompany()
					.getId(), order.getBaseUserByUserId().getId(),
		...
		NewStatusEvent event = new NewStatusEvent(order.getId(),
				order.getStatusId(), statusId);
		EventManager.process(event);
	...
	/**
	 * To be called from the http api, this simply looks for lines in the order
	 * that lack some fields, it finds that info based in the item.
	 * 
	 * @param dto
	 *            order with lines to pricess
	 * @param entityId
	 *            entity id
	 */
		...
		for (OrderLineDTO line : dto.getLines()) {
			...
			if (line.getPrice() == null) {
				line.setPrice(itemBl.getPrice(dto.getUserId(),
						dto.getCurrencyId(), line.getQuantity(), entityId));
			}
	...
	private void audit(Integer executorId, Date date) {
		eLogger.audit(executorId, order.getBaseUserByUserId().getId(),
				Constants.TABLE_PUCHASE_ORDER, order.getId(),
				EventLogger.MODULE_ORDER_MAINTENANCE, EventLogger.ROW_UPDATED,
				null, null, date);
	}
	...
	public static boolean validate(OrderLineWS dto) {
		...
		if (dto.getTypeId() == null || dto.getDescription() == null
	...
	public void reviewNotifications(Date today) throws NamingException,
		...
		for (CompanyDTO ent : new CompanyDAS().findEntities()) {
			...
			for (int f = 0; f < totalSteps; f++) {
				try {
					pref.set(
							ent.getId(),
							new Integer(
									CommonConstants.PREFERENCE_DAYS_ORDER_NOTIFICATION_S1
											.intValue() + f));
					if (pref.isNull()) {
						stepDays[f] = -1;
					} else {
			...
			JdbcTemplate jdbcTemplate = (JdbcTemplate) Context
					.getBean(Context.Name.JDBC_TEMPLATE);
			...
			while (results.next()) {
				...
				if (activeUntil.compareTo(today) >= 0
					...
					/*
					 * / ok LOG.debug("Selecting order " + orderId + " today = "
					 * + today + " active unitl = " + activeUntil + " days = " +
					 * days);
					 */
				} else {
					/*
				...
				try {
					...
					MessageDTO message = notification.getOrderNotification(ent
							.getId(), new Integer(currentStep), user
							.getEntity().getLanguageIdField(), order
							.getActiveSince(), order.getActiveUntil(), user
							.getEntity().getUserId(), order.getTotal(), order
							.getCurrencyId());
	...
	public TimePeriod getDueDate() {
		...
		if (order.getDueDateValue() == null) {
			...
			if (order.getBaseUserByUserId().getCustomer().getDueDateValue() == null) {
				...
				ConfigurationBL config = new ConfigurationBL(order
				...
				retValue.setUnitId(order.getBaseUserByUserId().getCustomer()
						.getDueDateUnitId());
				retValue.setValue(order.getBaseUserByUserId().getCustomer()
						.getDueDateValue());
			}
		} else {
		...
		try {
			preference.set(order.getUser().getEntity().getId(),
					CommonConstants.PREFERENCE_USE_DF_FM);
		} catch (EmptyResultDataAccessException e) {
	...
	public Integer[] getListIds(Integer userId, Integer number, Integer entityId) {
		...
		List<Integer> result = orderDas
				.findIdsByUserLatestFirst(userId, number);
		return result.toArray(new Integer[result.size()]);
	}

	public Integer[] getListIdsByItemType(Integer userId, Integer itemTypeId,
			Integer number) {

		List<Integer> result = orderDas.findIdsByUserAndItemTypeLatestFirst(
				userId, itemTypeId, number);
		return result.toArray(new Integer[result.size()]);
	}
	...
	public void updatePeriods(Integer languageId, OrderPeriodDTO periods[]) {
		for (OrderPeriodDTO period : periods) {
			orderPeriodDAS.save(period).setDescription(period.getDescription(),
					languageId);
	...
	public OrderLineWS getOrderLineWS(Integer id) {
		...
		OrderLineWS retValue = new OrderLineWS(line.getId(), line.getItem()
				.getId(), line.getDescription(), line.getAmount(),
				line.getQuantity(), line.getPrice() == null ? null
						: line.getPrice(), line.getCreateDatetime(),
				line.getDeleted(), line.getOrderLineType().getId(),
				line.getEditable(), line.getPurchaseOrder().getId(),
				line.getUseItem(), line.getVersionNum(),
				line.getProvisioningStatusId(), line.getProvisioningRequestId());
		return retValue;
	}
	...
	public OrderLineDTO getOrderLine(OrderLineWS ws) {
		...
		dto.setProvisioningStatus(provisioningStatusDas.find(ws
	...
	public List<OrderLineDTO> getOnetimeOrderLines(Integer userId,
			Integer itemId) {
		return orderLineDAS.findOnetimeByUserItem(userId, itemId);
	}

	public List<OrderLineDTO> getOnetimeOrderLines(Integer userId,
			Integer itemId, Integer months) {
		return orderLineDAS.findOnetimeByUserItem(userId, itemId, months);
	}

	public List<OrderLineDTO> getOnetimeOrderLinesByParent(
			Integer parentUserId, Integer itemId, Integer months) {
		return orderLineDAS.findOnetimeByParentUserItem(parentUserId, itemId,
				months);
	}
	...
	public void updateOrderLine(OrderLineWS dto) {
		...
		if (dto.getQuantity() != null
				&& (BigDecimal.ZERO.compareTo(dto.getQuantityAsDecimal()) == 0)) {
			...
			line.setProvisioningStatus(provisioningStatusDas.find(dto
					.getProvisioningStatusId()));
	 ...
	 * @return
	 */
	public static OrderDTO getOrCreateCurrentOrder(Integer userId,
			Date eventDate, Integer currencyId, boolean persist) {
		...
		if (currentOrderId == null) {
			...
			currentOrderId = co.create(eventDate, currencyId,
					new UserBL().getEntityId(userId));
	...
	public void setMainSubscription(Integer executorId) {
		...
		if (oldCurrent == null || !oldCurrent.equals(order.getId())) {
			...
			if (oldCurrent != null) {
				...
				if (old == null) {
					LOG.error("Can not find order to set 'is_current' off "
							+ oldCurrent);
				} else {
	...
	private void unsetMainSubscription(Integer executorId) {
		...
		if (order.getIsCurrent().intValue() == 1) {
			eLogger.audit(executorId, order.getBaseUserByUserId().getId(),
					Constants.TABLE_PUCHASE_ORDER, order.getId(),
					EventLogger.MODULE_ORDER_MAINTENANCE,
					EventLogger.ORDER_MAIN_SUBSCRIPTION_UPDATED, null, null,
					null);
			// this is the new subscription order
	...
	public void addRelationships(Integer userId, Integer periodId,
			Integer currencyId) {
		if (periodId != null) {
			OrderPeriodDTO period = orderPeriodDAS.find(periodId);
	...
	public OrderDTO getDTO(OrderWS other) {
		...
		retValue.setOrderBillingType(new OrderBillingTypeDAS().find(other
				.getBillingTypeId()));
		...
		retValue.setPayPlan(other.getPayPlan());
		...
		if (other.getPricingFields() != null) {
			...
			pf.addAll(Arrays.asList(PricingField.getPricingFieldsValue(other
					.getPricingFields())));
	...
	public void setProvisioningStatus(Integer orderLineId,
		...
		line.setProvisioningStatus(provisioningStatusDas
				.find(provisioningStatus));
		LOG.debug("order line " + line + ": updated provisioning status :"
		...
		eLogger.auditBySystem(order.getBaseUserByUserId().getCompany().getId(),
				...
				EventLogger.MODULE_PROVISIONING,
				EventLogger.PROVISIONING_STATUS_CHANGE, oldStatus, null, null);
	}

	public static OrderDTO createAsWithLine(OrderDTO order, Integer itemId,
			Double quantity) {
		return createAsWithLine(order, itemId,
				new BigDecimal(quantity).setScale(CommonConstants.BIGDECIMAL_SCALE,
	...
	public static OrderDTO createAsWithLine(OrderDTO order, Integer itemId,
		...
		newOrder.setOrderStatus(new OrderStatusDAS()
				.find(CommonConstants.ORDER_STATUS_ACTIVE));
